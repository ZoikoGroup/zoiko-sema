        ...
        <div className={`mt-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 transition-all duration-1000 delay-300 transform ${reportVisible ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0'}`}>
          {concerns.map((concern, idx) => (
            ...
            >
              ...
              <div className="pt-8">
                <Link href={concern.link} className={`inline-flex items-center justify-center w-full sm:w-auto px-4 py-2 rounded-full text-xs font-bold font-sans transition-all duration-200 transform hover:scale-[1.02] ${concern.bgClass}`}>
                  {concern.btnText}
                </Link>
              </div>
            </div>
      ...
      >
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 flex flex-col items-center text-center">
          ...
          <div className={`mt-10 flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-6 w-full sm:w-auto transition-all duration-1000 delay-400 transform ${ctaVisible ? 'translate-y-0 opacity-100' : 'translate-y-16 opacity-0'}`}>
            <Link href="#review" className="inline-flex items-center justify-center w-full sm:w-auto px-6 py-3 bg-white hover:bg-slate-50 text-indigo-800 font-sans text-sm font-bold rounded-full shadow-lg hover:-translate-y-0.5 active:translate-y-0 transition-all duration-200">
              Request security review
            </Link>
            
            <Link href="/data-processing-addendum" className="inline-flex items-center justify-center w-full sm:w-auto px-6 py-3 bg-white/10 hover:bg-white/20 text-white font-sans text-sm font-bold rounded-full border border-white/30 hover:border-white/50 hover:-translate-y-0.5 active:translate-y-0 transition-all duration-200">
              View Data Processing Addendum
            </Link>
            
            <Link href="/report-concern" className="inline-flex items-center justify-center w-full sm:w-auto px-4 py-2 text-white hover:text-purple-200 font-sans text-sm font-bold border-b border-white/40 hover:border-white transition-all duration-200">
              Report a concern
            </Link>
          </div>
