...
export default function TeamWayOfWorking() {
  // Separate observer references to cascade entrance reveals smoothly on scroll
  const { ref: headRef, inView: headInView } = useInView(0.2);
  const { ref: gridRef, inView: gridInView } = useInView(0.1);
  const { ref: ctaRef, inView: ctaInView } = useInView(0.05);
  ...
  const useCases: UseCaseCard[] = [
    {
      ...
      icon: (
        <img className='w-6 h-6 transition-transform duration-300 group-hover:scale-110 brightness-200' src="/TeamCollaboration/Frame (7).png" alt="" />
      ),
    },
    {
      ...
      icon: (
        <img className='w-6 h-6 transition-transform duration-300 group-hover:scale-110 brightness-200' src="/TeamCollaboration/Frame (9).png" alt="" />
      ),
    },
    {
      ...
      icon: (
        <img className='w-6 h-6 transition-transform duration-300 group-hover:scale-110 brightness-200' src="/TeamCollaboration/Frame (8).png" alt="" />
      ),
    },
    {
      ...
      icon: (
        <svg className="size-6 text-indigo-200 transition-transform duration-300 group-hover:scale-110" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
  ...
  return (
    <>
      <style>{`
        ...
        .ww-hidden  { opacity: 0; transform: translateY(36px); }
        .ww-visible { animation: wwFadeUp .65s cubic-bezier(.22,1,.36,1) forwards; }

        /* ── Card Custom Layer Interactions ── */
        .ww-card {
        ...
        .ww-card:hover {
          ...
          box-shadow: 0 20px 38px rgba(15,15,40,0.08);
        }

        @media (prefers-reduced-motion: reduce) {
          .ww-hidden, .ww-visible { opacity: 1 !important; transform: none !important; animation: none !important; }
      ...
      <section className="w-full bg-white dark:bg-slate-900 py-20 font-sans antialiased text-slate-900 dark:text-white transition-colors duration-300">
        <div className="max-w-[1248px] mx-auto px-6 md:px-8 flex flex-col items-center gap-12">
          ...
          {/* Header Block Container — Reveals upon viewport scroll entrance */}
          <div 
            ref={headRef}
            className={`max-w-[720px] w-full text-center flex flex-col items-center space-y-3 ww-hidden ${
              headInView ? "ww-visible" : ""
            }`}
          >
          ...
          {/* Use Cases Grid Layout Structure — Floats up cascadingly */}
          <div 
            ref={gridRef}
          ...
          >
            {useCases.map((useCase, index) => (
              <div 
                key={index}
                className={`group ww-card bg-white dark:bg-slate-800/40 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-[0px_1px_2px_0px_rgba(16,22,60,0.06)] flex flex-col sm:flex-row overflow-hidden ww-hidden ${
                  gridInView ? "ww-visible" : ""
                }`}
                style={{
          ...
          <div 
            ref={ctaRef}
            className={`pt-4 ww-hidden ${ctaInView ? "ww-visible" : ""}`}
