...
// e.g. "/images/terms-of-service/hero-bg.png"
  ...
  return (
    <>
      <style>{`
        ...
        .tos-hidden  { opacity: 0; transform: translateY(26px); }
        .tos-visible { animation: tosFadeUp .6s cubic-bezier(.22,1,.36,1) forwards; }
        ...
        @media (prefers-reduced-motion: reduce) {
          .tos-hidden { opacity: 1 !important; transform: none !important; }
          .tos-visible { animation: none !important; }
        }
      `}</style>
      ...
      >
        <div className="mx-auto w-full max-w-3xl px-6 md:px-10 text-center">
          ...
          <div
            ref={badgeRef}
            className={`tos-hidden ${badgeIn ? "tos-visible" : ""} inline-flex items-center gap-2 rounded-full bg-white px-4 py-2 mb-6`}
          >
          ...
          <h1
            ref={headingRef}
            className={`tos-hidden ${headingIn ? "tos-visible" : ""} text-[clamp(28px,4vw,38px)] font-bold tracking-tight text-white mb-4`}
          ...
          <p
            ref={copyRef}
            className={`tos-hidden ${copyIn ? "tos-visible" : ""} text-[14.5px] leading-relaxed mb-9 max-w-[620px] mx-auto`}
            style={{ color: "#C7C9DE", animationDelay: "0.1s" }}
          >
            Review the terms that govern access to and use of Zoiko Sema,
            including accounts, workspaces, subscriptions, AI features, user
            content, acceptable use, and related policies.
          </p>
